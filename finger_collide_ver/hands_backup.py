...
class Hands:
    def __init__(self):
        ...
        # -1: not present, 0: up, 1: down
        # 0-4 = left thumb-pinky, 5-9 = right thumb-pinky
        self.finger_state = [-1 for i in range(10)]
        
        # finger on which key
        self.finger_key = [-1 for i in range(10)]  

        # prev fingertip pos
        self.finger_tip_pos = [-1 for i in range(10)]
    ...
    def draw_fingertips(self, img):
        ...
        for hand_id, hand in enumerate(self.handedness_list):
            ...
            for finger in range(5):
                ...
                text = f"{finger+1} ({self.finger_state[finger_id]}): x {x*100:.1f}; y {y*100:.1f}; z {z*100:.1f}"
